...
import { IoChatbubbleOutline, IoTicketOutline, IoBusOutline } from "react-icons/io5";
import { FaRegLightbulb } from "react-icons/fa6";
import { MdOutlineDinnerDining, MdOutlineEmojiPeople, MdDownhillSkiing } from "react-icons/md";
                            ...
                            <div className={style['country-container']}>
                                <ul className={`nav nav-tabs border-0 ${style['country-nav-tabs']}`} id="tourPackageTab" role="tablist">
                                    {allTabs.map((tab) => (
                                        ...
                                        >
                                            <button
                                                className={`nav-link border-0 ${style['country-nav-link']} ${activeTab === tab.uuid_id ? 'active' : ''}`}
                                ...
                                {masonryLoading ? (
                                    <div className="text-center py-5">
                                        <BeatLoader color={"#0000FF"} loading={masonryLoading} size={15} />
                                    </div>
                                ) : filteredPackages.length === 0 ? (
                                    <div className="text-center py-5">
                                        <p>No packages match the selected filters. Try changing your selection.</p>
                                    </div>
                                ) : (
                                        ...
                                        <Masonry>
                                            {filteredPackages.map((pkg) => (
                                                <div key={pkg.id} className="masonry-item">
                                                    <img
                                                        src={pkg.photo_urls && pkg.photo_urls.length > 0 ? pkg.photo_urls[0] : '/images/placeholder.jpg'}
                                                    ...
                                                    <div className="tour-pakage-masonry-item-content">
                                                        ...
                                                        <Link href={`/tour-package/${pkg.id}`} className={style['r-button']}>Read More</Link>
                                                    </div>
                                                </div>
                                            ))}
                                        </Masonry>
