...
const DatePickerWithHover = ({ onClose, packageId , type = "package" }) => {
    ...
    const [packageDateRange, setPackageDateRange] = useState({ start: null, end: null, totalDays: 0 });
            ...
            try {
                ...
                if (!singlePackageData.start_date || !singlePackageData.end_date) {
                    ...
                    if (totalDays <= numberOfDays) {
                        ...
                        setSelectionDays(numberOfDays);
                        const defaultRange = getContinuousDateRange(startDate, numberOfDays);
    ...
    const displaySelectedDateAndTime = () => {
        ...
        return sortedDates.map((date, index) => (
            <span key={index}>{formatDate(date)}<span style={{marginLeft: '15px'}}></span></span>
        ));
    };
    ...
    const handleBookNow = () => {
        ...
        axios
          ...
          .catch((error) => {
            ...
            setError(error.response?.data?.message || "Failed to book package. Please try again.");
          });
      };
                    ...
                    <div className={` ${style["room-section"]} d-flex flex-column justify-content-between`}>
                        <div>
                            ...
                            {rooms.map((room, index) => (
                                <div key={room.id} className="d-flex flex-row justify-content-between col-12">
                                    <div className="me-xl-0 me-lg-5 me-5"><p>Room {room.id}</p></div>
                                    <div className='d-flex flex-xl-column flex-lg-row flex-md-row flex-column'>
                                        <div className="me-xl-0 me-xl-5 me-lg-4 me-md-4 me-5">
                                            ...
                                            <div style={{ display: 'flex', gap: '0px' }}>
                                                <button onClick={() => handleDecrement(room.id, 'adults')} className={style["numberdecrement"]}>-</button>
                                                <span style={{ padding: '1px 10px', fontSize: '20px', border: '1px solid' }}>{room.adults}</span>
                                                <button onClick={() => handleIncrement(room.id, 'adults')} className={style["numberincrement"]}>+</button>
                                            </div>
                                        ...
                                        <div>
                                            ...
                                            <div style={{ display: 'flex', gap: '0px' }}>
                                                <button onClick={() => handleDecrement(room.id, 'infant')} className={style["numberdecrement"]}>-</button>
                                                <span style={{ padding: '1px 10px', fontSize: '20px', border: '1px solid' }}>{room.infant}</span>
                                                <button onClick={() => handleIncrement(room.id, 'infant')} className={style["numberincrement"]}>+</button>
                                            </div>
                                    ...
                                    <div>
                                        ...
                                        <div style={{ display: 'flex', gap: '0px' }}>
                                            <button onClick={() => handleDecrement(room.id, 'children')} className={style["numberdecrement"]}>-</button>
                                            <span style={{ padding: '1px 10px', fontSize: '20px', border: '1px solid' }}>{room.children}</span>
                                            <button onClick={() => handleIncrement(room.id, 'children')} className={style["numberincrement"]}>+</button>
                                        </div>
                                        <p className="date_right_para">(Below  12 years)</p>
                                    </div>
                                    {index > 0 && (<MdOutlineCancel className='ms-3' size={20} color="red" style={{ cursor: 'pointer' }} onClick={() => handleRemoveRoom(room.id)}/>)}
                                </div>
                            ))}
                            {showRoomAlert && (<div className="room-alert">Maximum of 3 guests (excluding infants) reached. Please add a new room.</div>)}
                            <button className="bg-white col-12" style={{ border: 'none', color: '#3C99DC' }} onClick={handleAddRoom}>+ Add another room</button>
                        </div>
                    </div>
