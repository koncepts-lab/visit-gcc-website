...
export default function Top_container({ packageId }) {
    ...
    useEffect(() => {
        ...
        if (loginToken) {
            ...
            console.log("Using login token for fetching packages and vendor info.");
        } else if (registerToken) {
            setAuthToken(registerToken);
            console.log("Using register token for fetching packages and vendor info.");
        } else {
    ...
    useEffect(() => {
        const fetchPackageData = async () => {
            ...
            try {
                ...
                fetchVendorInfo(packageDetailsResponse.data.vendor_id); 
    ...
    const fetchVendorInfo = async (vendorId) => {
        if (!vendorId || !authToken) {
            console.error(`No vendor ID provided or authentication token missing.`);
            return;
        }
        ...
        try {
            ...
            const fetchedVendorName = `${userData.first_name} ${userData.last_name}`;
            setVendorName(fetchedVendorName);

        } catch (err) {
            console.error(`Error fetching vendor info for vendor ID ${vendorId}:`, err);
        }
    };

    const handleBookNowClick = () => {
        setIsDatePickerPopupOpen(true); // Updated state name
    };

    const handleCloseDatePickerPopup = () => {
        setIsDatePickerPopupOpen(false); // Updated state name
    };

    const openEnquiryForm = () => {
        setIsEnquiryFormOpen(true); // Updated state name
    };

    const closeEnquiryForm = () => {
        setIsEnquiryFormOpen(false); // Updated state name
    };
    ...
    const renderRatingCircles = () => {
        const averageRating = packageRatings?.average_rating ? parseFloat(packageRatings.average_rating) : 0;
        ...
        <div>
            <div className={`container ${style['container-package-details']}`}>
                <div className='row'>
                    <div className='col-md-7'>
                        <h3>{packageDetails.number_of_days} DAYS IN {packageDetails.name}</h3>
                        <p>
                            <Link href='#0'>{vendorName || 'Loading Vendor...'}</Link>
                        </p>
                        <div className={style['flex-package-details']}>
                            <span>
                                {packageRatings && renderRatingCircles()}
                                <p className='pt-2'>{packageRatings?.total_ratings || 0} reviews</p>
                            </span>
                            <span>
                    ...
                    <div className='col-md-6 col-lg-5'>
                        <div className={style['flex-package-details-right']}>
                            ...
                            <span>
                                <button className={style['btn-two']} onClick={openEnquiryForm}>Contact Seller</button>
            ...
            {isDatePickerPopupOpen && (
                <div className={style["popup-overlay"]}>
                    <EnhancedDatePicker packageId={packageId} onClose={handleCloseDatePickerPopup} />
                </div>
            )}
